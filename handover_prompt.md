...
以下の2機能を順番に実装してください。着手前に必ず `kpi_system.html` を Read して現在の行数・関数配置を確認すること。

---

### タスク①：承認通知（残業・有給）

管理者が残業申請・有給申請を **承認/却下** したとき、対象スタッフが次回ログイン時にトースト通知で気づけるようにする。

**実装方針（shiftNotify と同じパターン）**

```js
// state に追加する新フィールド（初期値 {}）
state.otNotify    // 残業：{si:[{date, hours, status:'approved'|'rejected'}]}
state.leaveNotify // 有給：{si:[{date, status:'approved'|'rejected'}]}
```

- `adminOt()` の承認/却下ボタン処理（`bindAdmin()` 内の `[data-ot-approve]` / `[data-ot-reject]` ハンドラ）で `otNotify[si]` にプッシュ
- `adminLeave()` の承認/却下処理（`bindLeaveAck()`）で `leaveNotify[si]` にプッシュ
- `bindLogin()` のスタッフログイン成功時に `otNotify[si]` / `leaveNotify[si]` があれば toast → クリア（shiftNotify の実装を参考に）
- `clearOperationData()` の消去対象に `otNotify`・`leaveNotify` を追加
- `removeStaff()` の再キー対象に `otNotify`・`leaveNotify` を追加（`reIntObj()` で OK）

---

### タスク②：業績サマリー Total ビューに達成率バー列を追加

`summaryPage()` の Total ビュー・アウトバウンドKPIテーブル（`actCols` を使っている部分）に、
各スタッフ行の右端に **達成率（進捗バー＋%）列** を追加する。

**実装方針**

- `actCols` の合計列として `契約数` の達成率を代表値として表示する（or スコア化）
- あるいは主要KPI（コール数）の達成率を 1 列追加する形でもよい
- テーブルヘッダに `<th>コール達成率</th>` を追加し、各行に以下を挿入：

```js
const callVal = sumKpi(i, 'コール数');
const callGoal = gData()[i]['コール数'] || 1;
const callPct = Math.min(100, Math.round(callVal / callGoal * 100));
const callCol = callPct >= 100 ? '#1D9E75' : callPct >= 70 ? '#BA7517' : '#D85A30';
// セル HTML
`<td style="min-width:100px">
  <div style="display:flex;align-items:center;gap:5px">
    <div class="progress-bar" style="flex:1;height:5px">
      <div class="progress-fill" style="width:${callPct}%;background:${callCol}"></div>
    </div>
    <span style="font-size:11px;color:${callCol};font-weight:600;white-space:nowrap">${callPct}%</span>
  </div>
</td>`
```

- Total 行（actTotRow）にも同様にチーム全体の達成率バーを追加する
...
state = {
  ...
  shifts,         // {si:{'YYYY-MM-DD':code}} シフト（code: 早|遅|夜|休。SHIFT_TYPES={c,label,bg,tx,start,end,nextDay} 参照）
...
### 未対応（次以降の候補）
- **【次セッション最優先】業績サマリー新ページ（案C）**：アウトバウンド+インバウンドを1ページに統合。詳細はプロンプト冒頭の「⚡ 次のセッションで最初にやること」参照
- 目標の複数月トレンドグラフ、承認の通知
- ファネル下流の0偏り（#7）／期間ナビの年範囲ハードコード（#8）／PWは平文保存（ハッシュ化推奨）
...
```
kpi_system.html   （単一ファイル・約190KB・約2896行）
```
