...
- クリック時にインライン2段階確認 → `shiftPrefs[nextMonthKey()]` を走査し、各スタッフ・各日について `start` が設定されている場合は PREF_SLOTS の `code` を `state.shifts[si][dateStr]` に書き込む（`null` = 絶対休み = `'休'` を書き込む）
- 未提出スタッフはスキップ（既存シフトを上書きしない）
- 完了後 `render()` して toast「シフト希望を反映しました」

---

### 提案F（次点）：シフト提出期限を管理者が変更できるようにする

現在 `REAL_D >= 20` でハードコードされている締切日を `state.shiftDeadline`（既定 `20`）で管理する。

**実装方針**

- `state.shiftDeadline = 20` を初期値として追加
- 管理者「シフト」タブに「提出期限（毎月 \_\_日）」の数値入力を追加（1〜28の範囲）
- `shiftPrefCardHTML(si)` / `computeAlerts()` の `REAL_D >= 20` を `REAL_D >= state.shiftDeadline` に置換
- `clearOperationData()` では `shiftDeadline` はリセットしない（設定値なので）
